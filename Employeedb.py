...
# ✅ Add Employee Route (Now Requires job_id)
...
async def add_employee():
    ...
    pdf_data = file.read()  # ✅ Fix: Remove 'await'
...
# ✅ Get Employees with Job Info
@employee_routes.route("/employees", methods=["GET"])
async def get_employees():
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(Employee.id, Employee.name, Employee.phone_number, Employee.email, Employee.pdf_resume, Employee.job_id, Job.title)
            .join(Job, Employee.job_id == Job.id)  # ✅ Join with Job table
        )
        ...
        employees_list = [
            {
                ...
                "job_id": emp.job_id,
                "job_title": emp.title  # ✅ Include job title
            }
