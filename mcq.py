...
from database import Job  # ✅ Removed duplicate AsyncSessionLocal import
...
async def fetch_job_details(job_id):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not job_details:
            ...
            return None, None  # ✅ Return None if job doesn't exist
...
async def generate_mcqs(job_title, job_skills):
    ...
    try:
        ...
        logging.info(f"📜 Raw Ollama Response: {raw_text}")  # ✅ Log raw response

        # ✅ Extract JSON using regex
        json_match = re.search(r"\[.*\]", raw_text, re.DOTALL)

        if json_match:
            json_text = json_match.group(0)  # Extract only JSON
            mcqs = json.loads(json_text)  # Convert to Python list
            return mcqs
        else:
...
@mcq_routes.route("/submit-mcq/<int:employee_id>", methods=["POST"])
async def submit_mcq_api(employee_id):
    """API to store MCQ results in the Employee table."""
    data = await request.get_json()
    logging.info(f"📥 Received Data: {data}")  # ✅ Log request data

    mcq_score = data.get("score")  
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            # ✅ Force score update
            employee.mcq_score = int(mcq_score)
            session.add(employee)  # ✅ Add the change
            await session.commit()
            await session.refresh(employee)  # ✅ Refresh to verify update
